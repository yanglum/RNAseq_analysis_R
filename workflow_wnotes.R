...
#no space means alternative code to what's above
...
# the following is not needed if BAM files are already sorted 
#   (can check by trying to index them)
...
#   use file.dir if BAM files came sorted
...
#   do this step at the end of this section
...
#dds.trans = DESeqDataSet(se.filter, design = ~ IP + Genotype) # 2 factor design
...
#alternative code for comparing every permutation of the 2 factors IP and Genotype
...
#dds = DESeqDataSet(se.filter, design = ~ Genotype + IP)
#   note this was stored in a different variable for transformation
dds = DESeq(dds) # normalization and dispersion estimate
...
#   not required if we want to compare every factor (ANOVA)

# Run the analysis
results.deseq2 = results(dds, contrast=contrast.deseq2, alpha=0.1)
#results.deseq2 = results(dds, alpha=0.1)
...
#design = model.matrix(~ Genotype + IP, dge$samples)
...
dge = calcNormFactors(dge) # uses TMM (upper quartile) to normalize
